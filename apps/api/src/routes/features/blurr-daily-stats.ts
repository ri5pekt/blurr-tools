...
export async function blurrDailyStatsRoutes(fastify: FastifyInstance) {
  // ─── POST /api/features/blurr-daily-stats/export ─────────────────────────

  fastify.post('/api/features/blurr-daily-stats/export', {
    ...
    for (const date of dates) {
      ...
      await queues.blurrDailyStats.add('export', { jobId: job.id, date }, {
        attempts: 5,
    ...
    log({
      ...
      message: `Blurr daily stats export triggered for ${dateFrom}${dateFrom !== dateTo ? ` → ${dateTo}` : ''} (${dates.length} day${dates.length > 1 ? 's' : ''})`,
      feature: FEATURE,
      jobId:   jobIds[0],
  ...
  // ─── GET /api/features/blurr-daily-stats/schedule ────────────────────────

  fastify.get('/api/features/blurr-daily-stats/schedule', {
    onRequest: [fastify.authenticate],
  }, async () => {
  ...
  // ─── PUT /api/features/blurr-daily-stats/schedule ────────────────────────

  fastify.put('/api/features/blurr-daily-stats/schedule', {
    onRequest: [fastify.authenticate],
  }, async (request, reply) => {
    ...
    log({
      ...
      message: 'Blurr daily stats schedule updated',
      feature: FEATURE,
      userId:  request.user.id,
      meta:    updates,
    })
