<template>
  <div class="blurr-daily-stats-view">

    <div class="page-header">
      <div class="header-title-row">
        <h1>Blurr Daily Stats</h1>
        <a
          v-if="sheetUrl"
          :href="sheetUrl"
          target="_blank"
      ...
      <p>Export daily Shopify stats (orders, fees, units, per-product breakdown) to Google Sheets.</p>
    </div>
    ...
    <section class="panel">
      ...
      <div v-else class="schedule-form">
        <p class="schedule-hint">
          When enabled, the export runs automatically each day and exports the previous day's stats.
        </p>
    ...
    <section class="panel">
      ...
      <JobLogsPanel feature="blurr_daily_stats_export" />
    </section>
...
const sheetUrl = import.meta.env.VITE_BLURR_DAILY_STATS_SHEET_URL as string | undefined
...
async function triggerExport() {
  ...
  try {
    const res = await apiClient.post<{ jobIds: string[]; count: number }>(
      '/features/blurr-daily-stats/export',
    ...
    toast.info(
      ...
      n === 1
        ? `Processing stats for ${dateFrom.value}…`
        : `Processing stats for ${dateFrom.value} → ${dateTo.value}…`,
    )
    queryClient.invalidateQueries({ queryKey: ['jobs', 'blurr_daily_stats_export'] })
  } catch (err: any) {
...
const { data: schedule, isLoading: scheduleLoading } = useQuery({
  queryKey: ['schedule', 'blurr_daily_stats_export'],
  queryFn:  async () => {
    const res = await apiClient.get<ScheduledExport | null>('/features/blurr-daily-stats/schedule')
    return res.data
  },
})
...
async function saveSchedule(enabled?: boolean) {
  ...
  try {
    ...
    await apiClient.put('/features/blurr-daily-stats/schedule', body)
    queryClient.invalidateQueries({ queryKey: ['schedule', 'blurr_daily_stats_export'] })
...
.select-input--sm  { width: 80px; }
.select-input--tz  { width: 230px; }
...
.btn-primary { background: var(--blurr-primary); color: #fff; border-color: var(--blurr-primary); }
.btn-primary:not(:disabled):hover { background: #9e3590; }

.btn-outline { background: #fff; color: #374151; border-color: #d1d5db; }
.btn-outline:not(:disabled):hover { background: #f9fafb; border-color: #9ca3af; }
...
.btn-danger-outline:not(:disabled):hover { background: #fef2f2; }
...
.alert .pi { flex-shrink: 0; margin-top: 1px; }
...
.schedule-hint { margin: 0; font-size: 0.8125rem; color: #6b7280; }

.schedule-row { display: flex; gap: 1.5rem; flex-wrap: wrap; }

.time-select-wrap { display: flex; align-items: center; gap: 0.375rem; }

.time-colon { font-size: 1rem; font-weight: 700; color: #374151; }

.schedule-actions { display: flex; gap: 0.75rem; flex-wrap: wrap; }

/* ─── Latest Job panel transition ──────────────────────────────────── */

.panel-appear-enter-active { transition: opacity 0.25s ease, transform 0.25s ease; }
.panel-appear-leave-active { transition: opacity 0.15s ease, transform 0.15s ease; }
.panel-appear-enter-from,
.panel-appear-leave-to { opacity: 0; transform: translateY(-6px); }

/* ─── Responsive ─────────────────────────────────────────────────────── */

@media (max-width: 600px) {
  .export-controls { flex-direction: column; align-items: stretch; }
  .picker-wrap { flex-direction: column; align-items: flex-start; }
  .range-picker :deep(.p-datepicker-input) { width: 100%; }
